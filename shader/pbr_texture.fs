...
void main() {
    ...
    vec3 normal = texture2D(material.normal, texCoord).rgb * 2.0 - 1.0;
    float metallic = texture2D(material.metallic, texCoord).r;
    float roughness = texture2D(material.roughness, texCoord).r;
    float ao = material.ao;
    vec3 fragNormal = normalize(TBN * normal);
